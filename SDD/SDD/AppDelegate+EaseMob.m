...
// SDK注册 APNS文件的名字, 需要与后台上传证书时的名字一一对应
    ...
    [[EaseMob sharedInstance] registerSDKWithAppKey:@"jofly#shangdodo"
                                       apnsCertName:nil
...
- (void)miPushRequestSuccWithSelector:(NSString *)selector data:(NSDictionary *)data{
    ...
    if ([selector isEqualToString:@"bindDeviceToken:"] && data[@"regid"])
    {
        NSString *idStr = [NSString stringWithFormat:@"%@",data[@"regid"]];
        
        NSUserDefaults * userDefaults = [NSUserDefaults standardUserDefaults];
        [userDefaults setObject:idStr forKey:@"MiPushUserId"];
        [userDefaults synchronize];
    }
    NSLog(@"请求成功(%@): %@", selector, data);
}

- (void)miPushRequestErrWithSelector:(NSString *)selector error:(int)error data:(NSDictionary *)data{
    
    NSLog(@"请求失败(%@): %@", selector, data);
}
...
//    [alert show];
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:userInfo[@"aps"][@"alert"]
                                                    message:userInfo[@"custom_content"]
                                                   delegate:nil
