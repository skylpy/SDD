//
//  PreferentialJoinDetailMoreViewController.m
//  SDD
//
...
@interface PreferentialJoinDetailMoreViewController ()
{
    /*- data -*/
...
@implementation PreferentialJoinDetailMoreViewController
...
- (NSDictionary *)getBaseInfo{
    ...
    NSArray *investmentAnalysisContent =  @[
                                            [model.referenceStore isKindOfClass:[NSNull class]] || model.referenceStore.length == 0?@"暂无":[NSString stringWithFormat:@"%@",model.referenceStore],
...
- (void)setupUI{
    ...
    preferentialDalail.text = ([model.joinConditions isKindOfClass:[NSNull class]] || model.joinConditions == nil || [model.joinConditions  isEqual: @""])?@"暂无":[NSString stringWithFormat:@"%@",model.joinConditions];
    [contentView addSubview:preferentialDalail];
    [preferentialDalail mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(lineView2.mas_bottom).with.offset(13);
        make.left.equalTo(contentView.mas_left).with.offset(10);
    ...
    if (model.fd_brandDescription!= nil) {
        ...
        joinAdvantageContent.text = [model.fd_joinAdvantage isKindOfClass:[NSNull class]]? @"暂无":[NSString stringWithFormat:@"%@",model.fd_joinAdvantage];
    }
    ...
    if (![model.planYearExtension isEqual:[NSNull null]]) {
        ...
        annualplanDalail.text = [model.joinProcess isKindOfClass:[NSNull class]]? @"暂无":model.joinProcess;
    }
    //annualplanDalail.text = model.planYearExtension;
    annualplanDalail.textColor = lgrayColor;
    annualplanDalail.numberOfLines = 2;
...
#pragma mark
#pragma mark 相关品牌推荐点击
- (void)topImageClick:(id)sender
{
    UITapGestureRecognizer *tap = sender;
    NSLog(@"%@ ---- %ld",sender,(long)tap.view.tag);
    NSLog(@"%@",dictData[@"recommendList"][tap.view.tag]);
    
    NSDictionary *dic = dictData[@"recommendList"][tap.view.tag];
    
    NSInteger brandType = [dic[@"type"] integerValue];
    
    if (brandType == 1) {
        
        PreferentialJoinDetailViewController *jdVC = [[PreferentialJoinDetailViewController alloc]init];
        jdVC.brandId = dic[@"brandId"];
