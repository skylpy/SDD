//
//  PreferentialJoinDetailViewController.m
//  SDD
//  优惠加盟
...
@interface PreferentialJoinDetailViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    /*- data -*/
...
@implementation PreferentialJoinDetailViewController
...
//    view_zxlq.backgroundColor = [UIColor redColor];
    ...
    [view_zxlq mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(viewWidth, 70));
    }];
...
//        }
    ...
    discoLabel.textColor = dblueColor;
    discoLabel.font = midFont;
    discoLabel.numberOfLines = 2;
    ...
    [discoLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(receiveBtn.mas_left).with.offset(-10);
        make.height.equalTo(receiveBtn.mas_height);
    }];
    discoLabel.text = [model.preferentialContent isEqual:[NSNull null]]?@"": model.preferentialContent;
...
//    
    //折扣
//    UILabel * Label_zk= [[UILabel alloc] init];
//    Label_zk.textColor = lgrayColor;
//    Label_zk.font = midFont;
//    Label_zk.text = model.fd_discountCoupons[@"maxPreferentialStr"];
//    [view_zxlq addSubview:Label_zk];
//    Label_zk.backgroundColor = [UIColor blueColor];
//    [Label_zk mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(discoLabel.mas_bottom).with.offset(1);
//        make.left.equalTo(view_zxlq.mas_left).with.offset(10);
//        
//    }];
    ...
    label_num.text = [NSString stringWithFormat:@"1/%ld",(unsigned long)_totalArr.count];
    
    //    下划线
    UIView *headCutOff = [[UIView alloc] init];
    headCutOff.backgroundColor = divisionColor;
    [contentView addSubview:headCutOff];
    [headCutOff mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(receiveBtn.mas_bottom).with.offset(15);
        make.left.equalTo(@0);
    ...
    for (int i=0; i<3; i++) {
        ...
        UILabel *labelHead = [[UILabel alloc] initWithFrame:CGRectMake(i*(viewWidth/3), 298, viewWidth/3, 20)];
...
//        labelHead.textColor = lgrayColor;
        ...
        UIView *headCutOff2 = [[UIView alloc] initWithFrame:CGRectMake(i*(viewWidth/3), 298, 0.5, 40)];
...
//    下划线
   UIView *cutOff = [[UIView alloc] init];
    ...
    Label_lq.textAlignment = NSTextAlignmentRight;
    Label_lq.text = [NSString stringWithFormat:@"%@人已领取",model.fd_joinedQty];
    [contentView addSubview:Label_lq];
...
//        make.size.mas_equalTo(CGSizeMake(5, 10));
    ...
    [discountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(Label_lq.mas_left).with.offset(-5);
        make.left.equalTo(PreferentialImage.mas_right).with.offset(10);
    }];
    
    UILabel * discountDetail1 = [[UILabel alloc] init];
    
    discountDetail1.font = midFont;
    discountDetail1.textColor = lgrayColor;
    discountDetail1.text =[model.preferentialContent isEqual:[NSNull null]]?@"": model.preferentialContent;
    [contentView addSubview:discountDetail1];
    
    [discountDetail1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(discountLabel.mas_bottom).with.offset(3);
        make.left.equalTo(PreferentialImage.mas_right).with.offset(10);
    }];
    ...
    [dateOfLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo( discountDetail1.mas_bottom).with.offset(3);
        make.left.equalTo(PreferentialImage.mas_right).with.offset(10);
    }];
    
    discountLabel.text = [model.preferentialContent isKindOfClass:[NSNull class]]?@"":model.preferentialContent;
    dateOfLabel.text = [dicount isKindOfClass:[NSNull class]]?@"":[NSString stringWithFormat:@"截止日期: %@",[Tools_F timeTransform:[dicount[@"endTime"] floatValue] time:days]];
    //    if (![dicount isEqual:[NSNull null]]) {
    //        discountLabel.text = [NSString stringWithFormat:@"%@折加盟优惠",dicount[@"discount"]];
    ////        discountDetail1.text = dicount[@"preferentialContent"];
    ...
    //        [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:25] range:NSMakeRange(0, 3)];
    ...
    [bgView3 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_greaterThanOrEqualTo(dateOfLabel.mas_bottom);
        make.top.mas_greaterThanOrEqualTo(PreferentialImage.mas_bottom).with.offset(10);
        make.left.equalTo(contentView.mas_left).with.offset(0);
        make.size.mas_equalTo(CGSizeMake(viewWidth, 10));
    }];
    ...
    preferentialDalail.text = [dicount[@"preferentialDescription"]isKindOfClass:[NSNull class]]||[dicount[@"preferentialDescription"]length]==0?@"":dicount[@"preferentialDescription"];
...
//    UIButton *draBtn;
//    if (![model.marketOutlook isEqual:[NSNull null]]) {
...
- (void)titleTap:(UIGestureRecognizer *)tap{
    ...
    switch (titleTag) {
        ...
        case 1000:
        {
            PreferentialJoinDetailMoreViewController *preVC = [[PreferentialJoinDetailMoreViewController alloc] init];
...
-(void)taplabelClick:(UITapGestureRecognizer *)tapl
{
    ...
    switch (titleTag) {
        case 100:
        {
            // 最新动态
            BranksIntroductionViewController *biVC = [[BranksIntroductionViewController alloc] init];
            NSDictionary * dict_dy;
            if ([dictData[@"dynamicList"] count]>0) {
                dict_dy = dictData[@"dynamicList"][0];
                //dynamicDalail.text = dict_dy[@"description"];
            }
            biVC.introductionContent = dict_dy[@"description"];
            biVC.NavTitle = @"最新动态";
            [self.navigationController pushViewController:biVC animated:YES];
        }
            break;
        ...
        case 130:
        {
            BranksIntroductionViewController *biVC = [[BranksIntroductionViewController alloc] init];
            biVC.NavTitle = @"可加盟区域";
            biVC.introductionContent = [model.fd_expandingRegion isKindOfClass:[NSNull class]]?@"":[NSString stringWithFormat:@"%@",model.fd_expandingRegion];
            [self.navigationController pushViewController:biVC animated:YES];
        }
        default:
            break;
