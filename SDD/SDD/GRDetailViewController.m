...
@interface GRDetailViewController ()<UIScrollViewDelegate,
UITableViewDataSource,UITableViewDelegate,BMKMapViewDelegate,BMKLocationServiceDelegate,
BMKGeoCodeSearchDelegate,UIAlertViewDelegate,UIWebViewDelegate>{
...
//    [self setNav:model.hd_house[@"houseName"]];
    ...
    titleLabel.text = NSLocalizedString(model.hd_house[@"houseName"], @"");
...
- (void)finish{
    if (self.deliverInt == 1) {
        ...
        for (MapListMenu *menu in window.subviews) {
...
//    播放图片若有视频
    ...
    if (![model.hd_images[@"videoUrls"] isEqual:[NSNull null]]) {
        
        if ([model.hd_images[@"videoUrls"] count] > 0 ) {
            UIImageView *PlayAVIcon = [[UIImageView alloc] init];
            PlayAVIcon.image = [UIImage imageNamed:@"bofang"];
            [_headImage addSubview:PlayAVIcon];
            
            [PlayAVIcon mas_makeConstraints:^(MASConstraintMaker *make) {
                
                make.center.equalTo(_headImage);
    ...
    surroundingString = [NSString stringWithFormat:@"周边人口数:  %@",
                      model.hd_house[@"surroundingPersons"] == nil? @"":model.hd_house[@"surroundingPersons"]];
    ...
    preferential.arrowImgView.image = [UIImage imageNamed:@""];
    preferential.appDiscount.text = [NSString stringWithFormat:@"%@",model.hd_housePreferential[@"content"]];
    preferential.remainArea.text = [NSString stringWithFormat:@"可租:  %@m²",model.hd_house[@"rentArea"]];
    ...
    switch ([model.hd_house[@"merchantsStatus"] intValue]) {
        case 0: notifyLabel.text = @"意向登记期通知我";
            break;
        case 1: notifyLabel.text = @"意向金收取期通知我";
            break;
        case 2: notifyLabel.text = @"转定签约期通知我";
            break;
        case 3: notifyLabel.text = @"开业通知我";
            break;
        default: notifyLabel.text = @"";
            break;
    }
...
//    HouseLooking *houseLooking = [[HouseLooking alloc] init];
//    
//    float houseLooking_h = 0;
//    if (![model.hd_houseShowings isEqual:[NSNull null]]) {
//        
//        if ([model.hd_houseShowings[@"showingsEndtime"] integerValue] != 0) {
//            houseLooking_h = 80;
//            
//            houseLooking.lookingHouseJoin.text = @"项目考察团报名中";
//            houseLooking.lookingHouseTime.text = [NSString stringWithFormat:@"活动时间:  %@",
//                                                  [Tools_F timeTransform:[model.hd_houseShowings[@"showingsEndtime"] intValue] time:days]];
//            [houseLooking.lookingHouse addTarget:self action:@selector(houseLooking:) forControlEvents:UIControlEventTouchUpInside];
//        }
//        else {
//            [houseLooking.lookingHouse removeFromSuperview];
//        }
//    }
//    
//    [bg_scrollView addSubview:houseLooking];
//    [houseLooking mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(notifyMe_bg.mas_bottom).offset(houseLooking_h==0?0:10);
//        make.left.equalTo(bg_scrollView.mas_left);
//        make.width.equalTo(bg_scrollView.mas_width);
//        make.height.mas_equalTo(houseLooking_h);
//    }];
    /*------------------------------ 项目参数 ------------------------------*/
    ...
    [houseParaTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(notifyMe_bg.mas_bottom).offset(10);
        make.left.equalTo(bg_scrollView.mas_left);
...
//    aiotText.numberOfLines = 0;
    ...
    UIWebView * aiotWebText = [[UIWebView alloc] init];
    aiotWebText.delegate = self;
    aiotWebText.scalesPageToFit = NO;
    
    float  attractInvestmentTitle_h = 0;
    float  attractInvestmentContent_h = 0;
    
    
    if (![model.hd_house[@"merchantsFormat"] isEqual:[NSNull null]]) {
        
        [aiotWebText loadHTMLString:model.hd_house[@"merchantsFormat"] baseURL:nil];
        if (![model.hd_house[@"merchantsFormat"] isEqualToString:@""]) {
            
            attractInvestmentTitle_h = 40;
            attractInvestmentContent_h = 10;
            //attractInvestmentWebTitle_h = 130;
        }
    ...
    [attractInvestmentObjectContent addSubview:aiotWebText];
    [aiotWebText mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(@(attractInvestmentWebTitle_h));
    }];
    
    [attractInvestmentObjectContent mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(aiotWebText.mas_bottom).with.offset(attractInvestmentContent_h);
    }];
...
//
    ...
    consultantTitle.arrowImgView.image = [UIImage imageNamed:@""];
...
//    [consultantTitle addGestureRecognizer:consultantTap];
    ...
    if (![model.hd_consultantList isEqual:[NSNull null]]) {
        ...
        consultantTitle.theTitle.text = [NSString stringWithFormat:@"项目招商经理"];//,[model.hd_consultantList count]
        consultantTitle_h = 40;
        consultantContent_h = [model.hd_consultantList count]<2?[model.hd_consultantList count]*70:2*70;
    }
    ...
    mainStoreListTitle.theTitle.text = @"主力店进驻";
...
//        else
//        {
//            mainStoreListTitle.theTitle.text = @"主力品牌进驻中";
//            mainStoreListTitle.arrowImgView.image = [UIImage imageNamed:@""];
//            mainStoreListTitle.userInteractionEnabled =NO;
//            
...
//        }
    ...
    latelyListTitle.theTitle.text = @"最近品牌签约";
    ...
    switch ([model.hd_house[@"merchantsStatus"] intValue]) {
        ...
        case 3: latelyListTitle.theTitle.text = @"最近签约品牌";
            break;
        default: latelyListTitle.theTitle.text = @"已入驻品牌";
            break;
    }
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    //获取页面高度（像素）
    NSString * clientheight_str = [webView stringByEvaluatingJavaScriptFromString:@"document.body.scrollHeight"];
    ...
    //再次设置WebView高度（点）
    attractInvestmentWebTitle_h = height-20;
    NSLog(@"%f",attractInvestmentWebTitle_h);
...
-(void)test{
    ...
    paraContent = @[
                    model.hd_house[@"commercialArea"] == nil?
                    @"暂无":[NSString stringWithFormat:@"%@",model.hd_house[@"commercialArea"]],
                    model.hd_house[@"floor"] == nil?
                    @"暂无":[NSString stringWithFormat:@"%@",model.hd_house[@"floor"]],
                    model.hd_house[@"lease"] == nil?
                    @"暂无":[NSString stringWithFormat:@"%@",model.hd_house[@"lease"]]
                    ];
...
//                      @"绿化率",
        ...
        paraContent = @[
                        ...
                        model.hd_house[@"managementFee"] == [NSNull null]?@"暂无":[NSString stringWithFormat:@"%@",model.hd_house[@"managementFee"]],
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (tableView.tag == 300) {
        ...
        return [model.hd_consultantList count]<2?[model.hd_consultantList count]:2;
    }else if (tableView.tag == 301){
...
- (void)getDiscount:(UIButton *)btn{
    ...
    getPreferentialVC.isOfficial = YES;
...
//    if ([model.hd_houseShowings[@"isApply"] integerValue]==1) {
//        
//        [self showErrorWithText:@"你已报名该看房团"];
//    }
//    else {
...
- (void)titleTap:(UIGestureRecognizer *)tap{
    ...
    switch (titleTag) {
        ...
        case 130:
        {
//            PanoramicMapViewController *psVC = [[PanoramicMapViewController alloc]init];
//            
//            psVC.titleTStr = model.hd_house[@"houseName"];
////            psVC.showType = PanoShowTypeGEO;
//            psVC.latitude = model.hd_house[@"latitude"];
//            psVC.longitude = model.hd_house[@"longitude"];
//            [self.navigationController pushViewController:psVC animated:YES];
