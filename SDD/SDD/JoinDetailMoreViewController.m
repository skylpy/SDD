...
@interface JoinDetailMoreViewController(){
...
@end
@implementation JoinDetailMoreViewController
...
- (void)setupNav{
    ...
    [self setNav:@"更多详情"];//model.fd_brandName
...
//    UIButton *_headImage = [UIButton buttonWithType:UIButtonTypeCustom];
//    
//    // 图片尺寸设定
    NSString *cutString = [NSString stringWithCurrentString:model.fd_defaultImage SizeWidth:viewWidth*2];
//    [_headImage sd_setImageWithURL:[NSURL URLWithString:cutString] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"loading_b"]];
...
//    brandIntroductionTitle.theTitle.text = @"品牌介绍";
...
//    }];
    ...
    //
        ...
        [investmentAnalysisTitle mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(propertyDescriptionContent.mas_bottom);
            make.left.equalTo(contentView.mas_left);
            make.size.mas_equalTo(CGSizeMake(viewWidth, 40));
        }];
    ...
    [terminalShowTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(joinAdvantageContent.mas_bottom).with.offset(8);
        make.left.equalTo(contentView.mas_left);
...
//    // 自动scrollview高度
    [contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(productShowContent.mas_bottom).with.offset(60);
    }];
    ...
    if (_brandType == 1) {
        ...
        [chatButton setTitle:@"品牌咨询" forState:UIControlStateNormal];
        ...
        [reservationButton addTarget:self action:@selector(takePhone:) forControlEvents:UIControlEventTouchUpInside];
        reservationButton.enabled = YES;
        
        [bottomView addSubview:reservationButton];
...
#pragma mark -- 暂时隐藏品牌信息报错
//        UIButton *callButton = [UIButton buttonWithType:UIButtonTypeCustom];
//        [callButton setTitle:@"品牌信息报错" forState:UIControlStateNormal];
...
//    join.backgroundColor = [SDDColor colorWithHexString:@"#008fec"];
//    [join setTitle:@"我要加盟" forState:UIControlStateNormal];
//    [join addTarget:self action:@selector(joinIn:) forControlEvents:UIControlEventTouchUpInside];
//    [Tools_F setViewlayer:join cornerRadius:15 borderWidth:0 borderColor:nil];
...
- (void)joinIn:(UIButton *)btn{
    ...
    if (![GlobalController isLogin]) {
        ...
        // 我要加盟
        IJoinViewController *ijVC = [[IJoinViewController alloc] init];
        ijVC.brandId = model.fd_brandId;
        ijVC.number = model.fd_tel;
...
- (void)takePhone:(UIButton *)btn{
    ...
    if (![GlobalController isLogin]) {
        ...
        //        NSURL *telURL = [NSURL URLWithString:[NSString stringWithFormat:@"tel:%@",model.fd_phone]];
