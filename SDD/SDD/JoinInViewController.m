...
@interface JoinInViewController()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,
...
//    NSInteger industryCategoryId1;                 // 行业id
    ...
    NSNumber *propertyTypeCategoryId;             // 品牌性质
}
...
- (void)requestDimensionality{
    ...
    NSMutableArray *Filter,*Filter2,*Filter3,*Filter4;
...
- (void)viewDidLoad{
    ...
    currentType = 0;
    otherTitles = @[@"需求面积",@"投资金额",@"物业类型",@"品牌性质"];
...
- (void)setupNav{
    ...
    all.tag = 100;
    [all setTitle:@"全部品牌" forState:UIControlStateNormal];
    ...
    all.selected = YES;
    [all addTarget:self action:@selector(allBrand:) forControlEvents:UIControlEventTouchUpInside];
    ...
    some.tag = 101;
    [some setTitle:@"优惠品牌" forState:UIControlStateNormal];
    ...
    [some addTarget:self action:@selector(someBrand:) forControlEvents:UIControlEventTouchUpInside];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    /*- 投资额度 -*/
    cell.investmentAmounts.text = [NSString stringWithFormat:@"%@",model.f_investmentAmountCategoryName];
    /*- 行业 -*/
    cell.industry.text = model.f_industryCategoryName;
    /*- 门店数量 -*/
    cell.franchiseesAmounts.text = [NSString stringWithFormat:@"%@",model.f_storeAmount];
...
- (void)menu:(DOPDropDownMenu *)menu didSelectRowAtIndexPath:(DOPIndexPath *)indexPath{
    ...
    if (indexPath.item >= 0) {
        ...
        if (indexPath.column == 1) {
            ...
            _otherInFilter = [_otherFilter objectAtIndex:indexPath.row];
        }
    }
}
    ...
    for (int i = 0; i < [indexPaths count]; i++) {
        ...
        for (NSIndexPath *indexPath in mu) {
            switch (i) {
                ...
                case 3:
                {
                    propertyTypeCategoryId = ((CategoryCell *)[[_otherFilter objectAtIndex:0]
                                                               objectAtIndex:indexPath.item]).areaCategoryId;
                }
                    break;
            }
