...
- (void)createNameButtons
{
    ...
    NSInteger len =[_viewArray count];
    //每个tab偏移量
    CGFloat xOffset = kWidthOfButtonMargin;
    CGFloat itemWidth = self.bounds.size.width/len;
    for (int i = 0; i < len; i++) {
...
//        topScrollViewContentWidth += kWidthOfButtonMargin+(self.bounds.size.width/4 - kWidthOfButtonMargin);
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset,0,
                                    itemWidth, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += itemWidth + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, itemWidth, _shadowImage.size.height);
            button.selected = YES;
        }
