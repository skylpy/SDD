...
@interface ThemeApplyViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation ThemeApplyViewController
...
//                            @"brandName":brandName};
    NSDictionary * dict = @{@"phone":telPhone,
                            ...
                            @"code":code};
    //@"/forums/addSignup.do"
    [HttpTool postWithBaseURL:SDD_MainURL Path:@"/activityForums/addSignup.do" params:dict success:^(id JSON) {
...
-(void)requestPhoneData
{
    ...
    [HttpTool postWithBaseURL:SDD_MainURL Path:@"/sms/user/sendForumsSignupCode.do" params:@{@"phone":telPhone} success:^(id JSON) {
...
-(void)createView
{
    ...
    NSString * originalString = [NSString stringWithFormat:@"已有%@人报名，报名截止至%@",signupQty,[Tools_F timeTransform:[_model.time intValue] time:dayszw]];
    ...
    [paintString addAttribute:NSForegroundColorAttributeName
                        ...
                        range:[originalString
                               rangeOfString:[NSString stringWithFormat:@"%@",signupQty]]];
