...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, 0, screenSize.width, screenSize.height)];
...
//            NSLog(@"rightTableView %@",self.rightTableView);
//        }else
...
- (void)animateTableViewShow:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        _leftTableView.frame = CGRectMake(self.origin.x+ScreenWidth*0.3, self.frame.origin.y, ScreenWidth*0.7, 0);
        [self.superview addSubview:_leftTableView];
        _rightTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, ScreenWidth*0.3, 0);
        [self.superview addSubview:_rightTableView];
        
        _leftTableView.alpha = 1.f;
        _rightTableView.alpha = 1.f;
        [UIView animateWithDuration:0.2 animations:^{
            _leftTableView.frame = CGRectMake(self.origin.x+ScreenWidth*0.3, self.frame.origin.y, ScreenWidth*0.7, _height);
            _rightTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, ScreenWidth*0.3, _height);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView == _leftTableView){
        ...
        [cell setSelectedTextColor:dblueColor];
//        cell.backgroundColor = [UIColor colorWithWhite:0.9 alpha:1.f];
    ...
    [cell setLayoutMargins:UIEdgeInsetsZero];//ios7下会引起崩溃
