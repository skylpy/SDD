...
+ (void)initialize
{
    ...
    appearance.titleFont_SIAlertView = [UIFont boldSystemFontOfSize:20];
...
- (void)updateTitleLabel
{
	if (self.title) {
		if (!self.titleLabel) {
			...
			self.titleLabel.font = self.titleFont_SIAlertView;
...
- (void)setTitleFont_SIAlertView:(UIFont *)titleFont_SIAlertView{
    if (_titleFont_SIAlertView == titleFont_SIAlertView) {
        return;
    }
    _titleFont_SIAlertView = titleFont_SIAlertView;
    self.titleLabel.font = titleFont_SIAlertView;
    [self invalidateLayout];
}
